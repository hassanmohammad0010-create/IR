...
from retrieval.inverted_retriever import InvertedRetriever
...
DATASETS = {
    "quora": {
        "index": "data/indexes/quora/inverted_index.pkl.gz",
    ...
    "msmarco": {
        "index": "data/indexes/msmarco/inverted_index.pkl.gz",
...
def query_term_coverage(idx, queries):
    total, found, oov = 0, 0, []
    for q in queries[:50]:
        for term in q["text"].split():
            ...
            if term in idx.index:
                ...
                oov.append(term)
    pct = found / max(total, 1) * 100
    print(f"\n  ℹ️  Query-term vocabulary coverage : {pct:.1f}%  ({found}/{total})")
    if oov:
        print(f"     OOV sample: {oov}")


def evaluate(ds_name, retriever, queries, qrels, cfg, total_docs):
    ...
    print(f"\n  📊 [{ds_name.upper()}] Inverted Index (IDF-weighted)")
...
def main():
    parser = argparse.ArgumentParser(description="Inverted Index — evaluate")
    parser.add_argument("--query_count", type=int, default=100)
    args = parser.parse_args()

    banner(f"Inverted Index Evaluation  |  query_count={args.query_count}")
    ...
    for ds_name, cfg in DATASETS.items():
        ...
        # total_docs = number of unique doc IDs across all posting lists
        total_docs = len({did for posting in idx.index.values() for did in posting})
        print(f"  ✔ Total documents  : {total_docs:,}")

        retriever = InvertedRetriever(idx, total_docs=total_docs)
        queries = load_queries(cfg["queries"], args.query_count)
        qrels = load_qrels(cfg["qrels"])

        section(f"[{ds_name.upper()}] Query-term Coverage")
        query_term_coverage(idx, queries)

        section(f"[{ds_name.upper()}] Evaluation")
        agg = evaluate(ds_name, retriever, queries, qrels, cfg, total_docs)
