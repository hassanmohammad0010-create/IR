...
DATASETS = {
    "quora": {
        "index": "data/indexes/quora/embedding_index.pkl.gz",
    ...
    "msmarco": {
        "index": "data/indexes/msmarco/embedding_index.pkl.gz",
...
def main():
    parser = argparse.ArgumentParser(description="Embedding — evaluate")
    parser.add_argument("--query_count", type=int, default=100)
    args = parser.parse_args()

    banner(f"Embedding Evaluation  |  query_count={args.query_count}")
    ...
    for ds_name, cfg in DATASETS.items():
        ...
        t0 = time.time()
        idx = EmbeddingIndex.load(str(index_path))
        print(f"  ✔ Documents     : {len(idx.doc_ids):,}")
        print(f"  ✔ Model         : {idx.model_name}")
        print(f"  ✔ Matrix shape  : {idx.matrix.shape}")
        print(f"  ✔ Memory usage  : {idx.matrix.nbytes / 1e6:.1f} MB")

        retriever = EmbeddingRetriever(idx)
