...
RAW_JSONL = ROOT / "datasets" / "dataset2" / "quora_docs.jsonl"
PROCESSED_JSONL = ROOT / "data" / "processed" / "quora_processed.jsonl"

INDEX_DIR = ROOT / "data" / "indexes" / "quora"
...
def step_preprocess(args: argparse.Namespace) -> None:
    ...
    logger.info("[1/4] Preprocessing Quora corpus")
...
def step_inverted(args: argparse.Namespace) -> None:
    ...
    logger.info(
        f"docs={count:,} vocab={idx.vocabulary_size():,} " f"time={time.time()-t0:.1f}s"
    )
...
def main() -> None:
    ...
    logger.info("Starting Quora build pipeline...")
